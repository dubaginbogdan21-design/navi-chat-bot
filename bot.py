...
russian_bad_words = ['мат', 'нецензурная', 'брань']  # замените на реальный список
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type == ChatType.PRIVATE:
        await update.message.reply_text(
            '🤖 Я использую DeepSeek для ответов.\n'
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Личные сообщения — отвечаем через DeepSeek
    if chat.type == ChatType.PRIVATE:
        ...
        try:
            response = await client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": f"Ты полезный ассистент по имени Navi Chat."},
    ...
    if not await check_bot_rights(update, context):
        # Если нет прав, просто игнорируем (можно уведомить один раз)
        return
...
def main():
    ...
    print("Бот Navi Chat (админ-версия) запущен...")
    app.run_polling()
